...
$somatotalqtd = 0;
    ...
    foreach ($unidades as $unidade_atual) {
        ...
        function enviaForm<?php echo $codigo; ?>($mes){
                        window.open('../../../model/financeiro/faturaconsolidada/editar.php?codigo=<?php echo $codigo; ?>&ano=<?php echo $ano?>&mes='+$mes,'<?php echo $codigo; ?>','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=yes, width=450, height=300');
   }
        </script>
...
//                $vl_procedimento = 0;
    ...
    $vl_procedimento  = str_ireplace(",", ".", $vl_procedimento );
                ...
                if (($codigo == "20") and ( $i == 2)) {
                 ...
                 if (($codigo == "53")and ( $i == 2) and ($desconto != "")){
 ...
 $total_geral = $total_geral - $valor_descontar;
                ...
                }elseif(($codigo == "53")and ( $i == 2) and ($desconto <= "")){
     $linha_desconto = '<tr><td>Desc.</td><td></td><td>' . number_format($valor_descontar, 2, ',', '.') . '</td><td></td></tr>';
                ...
                if(($codigo == "20") and ( $i == 2)OR ($codigo == "14")and ( $i == 2) or ($codigo == "53") and ( $i == 2)){
...
if(($codigo != 53) and ($codigo != "56") or ($codigo != "57")){
    $ex_valor = "$vl_procedimento,00";   
}
elseif(
        ($vl_procedimento > 0) and ($codigo == "53") 
        or 
        ($vl_procedimento > 0) and ($codigo == "56") 
        or 
        ($vl_procedimento > 0) and ($codigo == "57") 
        ){
...
<tr>
    <td>
      ';
                $vl_procedimento = trim($vl_procedimento);
                  echo  "$ex_valor";
